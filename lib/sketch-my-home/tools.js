...
export class ToolsManager {
        ...
        if (this.currentTool === 'select') {
            if (this.state.isMarquee) {
                ...
                for (const {item, orig} of this.state.origPropsArray) {
                    ...
                    } else if (item.type === 'room' || item.type === 'object' || (item.type === 'boundary' && item.x != null)) {
                        item.x = orig.x + dx;
                        item.y = orig.y + dy;
                ...
                const handleHit = this.getHandleHit(x, y);
                if (handleHit) {
                    if (handleHit.item.type === 'room' || handleHit.item.type === 'boundary' || handleHit.item.type === 'object') {
            ...
            // Threshold: if Shift held → always ortho; otherwise auto-snap within 15° of H or V
            ...
            if (forceOrtho || autoSnap) {
                if (forceOrtho ? Math.abs(dx) > Math.abs(dy) : nearHoriz) {
                    // Lock to horizontal — clamp Y to start
                    rawY = this.state.startY;
                } else {
                    // Lock to vertical — clamp X to start
                    rawX = this.state.startX;
                }
            }
            // Store which axis was locked so overlay can indicate it
    ...
    onMouseUp(e) {
        if (this.currentTool === 'select') {
            ...
            this.state.isDrawing = false;
            let rx = Math.min(this.state.startX, this.state.endX);
            let ry = Math.min(this.state.startY, this.state.endY);
            let rw = Math.abs(this.state.endX - this.state.startX);
            let rh = Math.abs(this.state.endY - this.state.startY);

            const site = getBoundaryRect(this.engine.scene);
            if (site) {
                const ix = Math.max(rx, site.minX);
                const iy = Math.max(ry, site.minY);
                const ix2 = Math.min(rx + rw, site.maxX);
                const iy2 = Math.min(ry + rh, site.maxY);
                rw = ix2 - ix;
                rh = iy2 - iy;
                rx = ix;
                ry = iy;
    ...
    onKeyDown(e) {
        ...
        if (e.key === 'Escape') {
            this.engine.clearSelection();
            if (this.state.isDrawing || (this.state.points && (this.currentTool === 'boundary' || this.currentTool === 'measure_area'))) {
                this.state.isDrawing = false;
    ...
    drawOverlay(ctx) {
        ...
        if (!this.state.isDrawing) {
            ...
            if (['wall', 'room', 'boundary', 'door', 'window', 'stairs', 'bed', 'table', 'measure', 'measure_area', 'hole'].includes(this.currentTool)) {
        ...
        if ((this.currentTool === 'measure_area' || this.currentTool === 'boundary') && this.state.points) {
            const points = this.state.points;
            const isBoundary = this.currentTool === 'boundary';
            ctx.save();
            ctx.strokeStyle = isBoundary ? 'rgba(245, 158, 11, 0.85)' : 'rgba(99, 102, 241, 0.7)';
            ctx.lineWidth = 2 / this.engine.scale;
            ctx.lineJoin = 'round';
            ctx.lineCap = 'round';
            ...
            ctx.fillStyle = isBoundary ? '#f59e0b' : '#6366f1';
