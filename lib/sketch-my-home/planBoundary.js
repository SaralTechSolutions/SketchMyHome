...
/**
 * Room outline: saved polygon `points`, or legacy rectangle as four corners (clockwise).
 * @returns {Array<{x:number,y:number}>|null}
 */
export function getRoomPoints(shape) {
    if (!shape || shape.type !== 'room') return null;
    if (Array.isArray(shape.points) && shape.points.length >= 3) {
        return shape.points;
    }
    return rectBoundaryToPoints(shape);
...
/** True when the room was drawn as a multi-vertex polygon (vertex handles like site boundary). */
export function isPolygonRoom(item) {
    return !!(item && item.type === 'room' && Array.isArray(item.points) && item.points.length >= 3);
}
