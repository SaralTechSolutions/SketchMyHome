...
ElementRegistry.register({
    ...
    draw(ctx, hw, hh, w, h, scale) {
        ...
        ctx.setLineDash([]);
        ctx.beginPath();
        ctx.moveTo(-hw * 0.55, -hh * 0.35);
        ctx.lineTo(hw * 0.55, hh * 0.35);
        ctx.moveTo(hw * 0.55, -hh * 0.35);
        ctx.lineTo(-hw * 0.55, hh * 0.35);
        ctx.stroke();
    }
...
ElementRegistry.register({
    ...
    draw(ctx, hw, hh, w, h, scale) {
        ...
        ctx.strokeRect(-hw, -hh, w, h);
        ctx.setLineDash([6 / scale, 4 / scale]);
        ctx.strokeRect(-hw * 0.65, -hh * 0.55, w * 0.55, h * 0.7);
        ctx.setLineDash([]);
    }
});
...
ElementRegistry.register({
    ...
    width: 75,
    height: 200,
    /** Nominal run width in feet (synced to pixel `width` on the scene object). */
    extraProps: { widthFt: 3 },
    draw(ctx, hw, hh, w, h, scale, shape, colors) {
        ...
        if (wf != null && Number.isFinite(wf)) {
            const fontSize = Math.min(11, Math.max(8, w / 8)) / scale;
            ctx.font = `600 ${fontSize}px Inter, sans-serif`;
            ctx.fillStyle = colors?.textColor || '#1e293b';
            ctx.textAlign = 'center';
            ctx.textBaseline = 'bottom';
            ctx.fillText(`${wf}' wide`, 0, -hh - 4 / scale);
        }
    }
});
