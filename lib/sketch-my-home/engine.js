...
export class CanvasEngine {
    constructor(canvasEl) {
        ...
        this.scale = 1;
        this.minViewScale = MIN_VIEW_SCALE;
        this.maxViewScale = MAX_VIEW_SCALE;
        /** Architectural drawing scale (denominator), e.g. 100 → "1:100". */
        this.planScaleDenominator = DEFAULT_PLAN_SCALE_DENOMINATOR;
        ...
        this.compColor = '#94a3b8'; // Slate-400
        this.wallColor = '#64748b'; // Slate-500
        this.compFill = 'rgba(148, 163, 184, 0.08)';
        this.baseText = '#94a3b8';
        this.gridColor = 'rgba(148, 163, 184, 0.05)';
        ...
        /** data URL → { img: HTMLImageElement, ready: boolean } for plan images on openings/frames */
        this._planImageCache = new Map();
    ...
    updateContrastColors() {
        const cssBg = typeof window !== 'undefined' ? getComputedStyle(document.body).getPropertyValue('--bg-canvas').trim() : '';
        const bg = this.bgColor || cssBg || '#ffffff';
    ...
    zoomAtPosition(x, y, newScale) {
        newScale = Math.max(this.minViewScale, Math.min(newScale, this.maxViewScale));
    ...
    setZoom(level) {
        const wx = (this.canvas.width / 2 - this.offsetX) / this.scale;
        const wy = (this.canvas.height / 2 - this.offsetY) / this.scale;
        this.zoomAtPosition(wx, wy, level);
    }

    /** Zoom relative to canvas center (world-space pivot). */
    zoomAtCanvasCenter(newScale) {
        const wx = (this.canvas.width / 2 - this.offsetX) / this.scale;
        const wy = (this.canvas.height / 2 - this.offsetY) / this.scale;
        this.zoomAtPosition(wx, wy, newScale);
    }
    ...
    /**
     * Loads a plan image from a data URL (cached). Triggers re-render when ready.
     * @returns {HTMLImageElement | null} ready image, or null while loading / invalid
     */
    ensurePlanImage(dataUrl) {
        if (!dataUrl || typeof dataUrl !== 'string') return null;
        if (!this._planImageCache.has(dataUrl)) {
            const img = new Image();
            ...
            img.src = dataUrl;
            this._planImageCache.set(dataUrl, entry);
        }
        const e = this._planImageCache.get(dataUrl);
        return e.ready && e.img.complete ? e.img : null;
    }

    /** Fill/stroke tints for wall openings & furniture (semi-transparent); images draw at full opacity on top. */
    getObjectPlanTint(subType, isInteractive, isSelected) {
        const a = isSelected ? 0.38 : 0.26;
        const s = isSelected ? 0.92 : 0.78;
        const t = {
            door: {
                fill: `rgba(59, 130, 246, ${a})`,
                stroke: `rgba(29, 78, 216, ${s})`,
                border: `rgba(29, 78, 216, ${0.55 * s})`,
            },
            window: {
                fill: `rgba(34, 211, 238, ${a})`,
                stroke: `rgba(8, 145, 178, ${s})`,
                border: `rgba(8, 145, 178, ${0.55 * s})`,
            },
            hole: {
                fill: `rgba(167, 139, 250, ${a})`,
                stroke: `rgba(109, 40, 217, ${s})`,
                border: `rgba(91, 33, 182, ${0.55 * s})`,
            },
            frame: {
                fill: `rgba(147, 51, 234, ${a})`,
                stroke: `rgba(88, 28, 135, ${s})`,
                border: `rgba(88, 28, 135, ${0.55 * s})`,
            },
            stairs: { fill: `rgba(251, 191, 36, ${a})`, stroke: `rgba(180, 83, 9, ${s})`, border: `rgba(180, 83, 9, ${0.5})` },
            bed: { fill: `rgba(244, 114, 182, ${a})`, stroke: `rgba(190, 24, 93, ${s})`, border: `rgba(190, 24, 93, ${0.45})` },
            table: { fill: `rgba(45, 212, 191, ${a})`, stroke: `rgba(15, 118, 110, ${s})`, border: `rgba(15, 118, 110, ${0.45})` },
            bookshelf: { fill: `rgba(251, 146, 60, ${a})`, stroke: `rgba(194, 65, 12, ${s})`, border: `rgba(194, 65, 12, ${0.45})` },
            commode: { fill: `rgba(125, 211, 252, ${a})`, stroke: `rgba(3, 105, 161, ${s})`, border: `rgba(3, 105, 161, ${0.45})` },
            washing_machine: { fill: `rgba(165, 243, 252, ${a})`, stroke: `rgba(8, 145, 178, ${s})`, border: `rgba(8, 145, 178, ${0.45})` },
            chair: { fill: `rgba(196, 181, 253, ${a})`, stroke: `rgba(91, 33, 182, ${s})`, border: `rgba(91, 33, 182, ${0.45})` },
            sofa: { fill: `rgba(253, 186, 116, ${a})`, stroke: `rgba(194, 65, 12, ${s})`, border: `rgba(194, 65, 12, ${0.45})` },
            text: { fill: `rgba(148, 163, 184, ${a})`, stroke: `rgba(71, 85, 105, ${s})`, border: `rgba(71, 85, 105, ${0.45})` },
        };
        const base = t[subType] || {
            fill: isInteractive ? `rgba(99, 102, 241, ${a})` : 'rgba(99, 102, 241, 0.15)',
            stroke: isSelected ? '#6366f1' : this.compColor,
        ...
        for (let i = this.scene.length - 1; i >= 0; i--) {
            ...
            if (item.type === 'room' || item.type === 'object') {
                if (item.type === 'room' && isPolygonRoom(item)) {
                    ...
                    if (pointInPolygon(x, y, pts)) return item;
                    const n = pts.length;
                    for (let i = 0, j = n - 1; i < n; j = i++) {
                        const dist = this.distToSegment({ x, y }, pts[i], pts[j]);
                ...
                const pts = getBoundaryPoints(item);
                if (pts && pts.length >= 3) {
                    if (pointInPolygon(x, y, pts)) return item;
        ...
        for (const item of this.scene) {
            if (item.type === 'room' || item.type === 'object') {
                if (item.type === 'room' && isPolygonRoom(item)) {
                    ...
                    const aMinX = Math.min(...xs);
                    const aMaxX = Math.max(...xs);
                    const aMinY = Math.min(...ys);
                    const aMaxY = Math.max(...ys);
                ...
                const pts = getBoundaryPoints(item);
                if (pts && pts.length) {
                    const aMinX = Math.min(...pts.map((p) => p.x));
                    const aMaxX = Math.max(...pts.map((p) => p.x));
                    const aMinY = Math.min(...pts.map((p) => p.y));
                    const aMaxY = Math.max(...pts.map((p) => p.y));
                    if (aMaxX >= minX && aMinX <= maxX && aMaxY >= minY && aMinY <= maxY) {
                        selected.push(item);
                    }
                }
    ...
    render(drawBackgroundAndGrid = true) {
        ...
        if (drawBackgroundAndGrid) {
            this.ctx.fillStyle = this.bgColor || (typeof window !== 'undefined' ? getComputedStyle(document.body).getPropertyValue('--bg-canvas').trim() : '') || '#ffffff';
        } else {
        ...
        if (drawBackgroundAndGrid && this.showGrid) {
            ...
            const gridBaseColor = this.isLightBg ? '0, 0, 0' : '255, 255, 255';
            
            // --- Minor Grid (1 Inch) ---
            if (this.scale > 1.8) {
                this.ctx.strokeStyle = `rgba(${gridBaseColor}, 0.03)`;
                this.ctx.lineWidth = 0.5 / this.scale;
                this.ctx.beginPath();
                const inchSize = this.gridSize / 12;
                const firstInchX = Math.floor(startX / inchSize) * inchSize;
                const firstInchY = Math.floor(startY / inchSize) * inchSize;
                for (let x = firstInchX; x <= endX; x += inchSize) {
                    if (Math.round(x % this.gridSize) === 0) continue; 
                    this.ctx.moveTo(x, startY);
                    this.ctx.lineTo(x, endY);
                }
                for (let y = firstInchY; y <= endY; y += inchSize) {
                    if (Math.round(y % this.gridSize) === 0) continue;
            ...
            // --- Major Grid (1 Foot) ---
            this.ctx.strokeStyle = `rgba(${gridBaseColor}, 0.08)`;
        ...
        // 3. Draw scene items — site boundary under everything else
        const boundaries = this.scene.filter((s) => s.type === 'boundary');
        const rest = this.scene.filter((s) => s.type !== 'boundary');
    ...
    drawRoom(shape, isInteractive, isSelected) {
        const pts = getRoomPoints(shape);
        const roomLabel = this.getRoomTypeLabel(shape.roomType);
        ...
        if (pts && pts.length >= 3) {
            ...
            this.ctx.fillText(roomLabel, cx, cy - 7 / this.scale);
            const wFt = this.pixelsToFeet(shape.width);
            const hFt = this.pixelsToFeet(shape.height);
            this.ctx.font = `${fontSize * 0.92}px Inter, sans-serif`;
            this.ctx.fillStyle = isInteractive ? 'rgba(71, 85, 105, 0.95)' : this.baseText;
            this.ctx.fillText(`${wFt} × ${hFt}`, cx, cy + 8 / this.scale);
            ...
            const wFt = this.pixelsToFeet(shape.width);
            const hFt = this.pixelsToFeet(shape.height);
            this.ctx.fillText(roomLabel, shape.x + shape.width / 2, shape.y + shape.height / 2 - 7 / this.scale);
            this.ctx.font = `${fontSize * 0.92}px Inter, sans-serif`;
            this.ctx.fillStyle = isInteractive ? 'rgba(71, 85, 105, 0.95)' : this.baseText;
            this.ctx.fillText(`${wFt} × ${hFt}`, shape.x + shape.width / 2, shape.y + shape.height / 2 + 8 / this.scale);
    ...
    getRoomTypeLabel(roomType) {
        ...
        if (roomType && labels[roomType]) return labels[roomType];
        return 'Room';
    }

    drawBoundary(shape, isInteractive, isSelected) {
        const pts = getBoundaryPoints(shape);
        this.ctx.save();
        this.ctx.fillStyle = isSelected ? 'rgba(245, 158, 11, 0.18)' : 'rgba(245, 158, 11, 0.08)';
        this.ctx.strokeStyle = isSelected ? '#f59e0b' : 'rgba(245, 158, 11, 0.85)';
        this.ctx.lineWidth = 2.5 / this.scale;
        this.ctx.setLineDash([10 / this.scale, 6 / this.scale]);
        ...
        if (pts && pts.length >= 3) {
            ...
            cx /= pts.length;
            cy /= pts.length;
        } else if (shape.x != null && shape.width != null) {
            this.ctx.fillRect(shape.x, shape.y, shape.width, shape.height);
            this.ctx.strokeRect(shape.x, shape.y, shape.width, shape.height);
            ...
            minPx = shape.x;
            minPy = shape.y;
            maxPx = shape.x + shape.width;
            maxPy = shape.y + shape.height;
        }
        this.ctx.setLineDash([]);

        this.ctx.fillStyle = isInteractive ? (isSelected ? '#f59e0b' : this.baseText) : this.baseText;
        const fontSize = 11 / this.scale;
        this.ctx.font = `600 ${fontSize}px Inter, sans-serif`;
        this.ctx.textAlign = 'center';
        this.ctx.textBaseline = 'middle';
        const label = `Site boundary · 1:${this.planScaleDenominator}`;
        this.ctx.fillText(label, cx, cy - 8 / this.scale);
        if (pts && pts.length >= 3 && Number.isFinite(maxPx - minPx)) {
            const wFt = this.pixelsToFeet(maxPx - minPx);
            const hFt = this.pixelsToFeet(maxPy - minPy);
            this.ctx.fillText(`${wFt} × ${hFt} (bbox)`, cx, cy + 8 / this.scale);
        } else if (shape.width != null) {
            const wFt = this.pixelsToFeet(shape.width);
            const hFt = this.pixelsToFeet(shape.height);
            this.ctx.fillText(`${wFt} × ${hFt}`, cx, cy);
        }
        ...
        } else if (isSelected && shape.x != null) {
            const r = 4 / this.scale;
            this.ctx.fillStyle = '#f59e0b';
            this.ctx.fillRect(shape.x - r, shape.y - r, r * 2, r * 2);
            this.ctx.fillRect(shape.x + shape.width - r, shape.y - r, r * 2, r * 2);
    ...
    /** Polygon footprint (angled sides) — same interaction model as site boundary. */
    drawPolygonStaircase(shape, isInteractive, isSelected) {
        ...
        this.ctx.closePath();
        this.ctx.fillStyle = isInteractive
            ? isSelected
                ? 'rgba(252, 165, 165, 0.42)'
                : 'rgba(254, 202, 202, 0.32)'
        ...
        this.ctx.strokeStyle = 'rgba(220, 38, 38, 0.28)';
        this.ctx.lineWidth = 1 / this.scale;
        for (let y = minPy - pad; y <= maxPy + pad; y += step) {
            this.ctx.beginPath();
            this.ctx.moveTo(minPx - pad, y);
            this.ctx.lineTo(maxPx + pad, y);
            this.ctx.stroke();
        }
        this.ctx.restore();

        this.ctx.strokeStyle = isInteractive
            ? isSelected
                ? 'rgba(220, 38, 38, 0.95)'
        ...
        cx /= n;
        cy /= n;
        const wf = shape.widthFt;
        if (wf != null && Number.isFinite(wf)) {
            this.ctx.fillStyle = isInteractive ? (isSelected ? '#b91c1c' : this.baseText) : this.exportWallColor;
            const fontSize = 11 / this.scale;
            this.ctx.font = `600 ${fontSize}px Inter, sans-serif`;
            this.ctx.textAlign = 'center';
            this.ctx.textBaseline = 'middle';
            this.ctx.fillText(`${wf}' wide`, cx, cy);
        }
    ...
    /** Purple wall frame; optional user image is drawn opaque inside the polygon. */
    drawPolygonFrame(shape, isInteractive, isSelected) {
        const pts = shape.points;
        const n = pts.length;
        const tint = this.getObjectPlanTint('frame', isInteractive, isSelected);
        const img = shape.imageDataUrl ? this.ensurePlanImage(shape.imageDataUrl) : null;
        ...
        if (img) {
            ...
            const iw = maxPx - minPx;
            const ih = maxPy - minPy;
            this.ctx.globalAlpha = 1;
            this.ctx.drawImage(img, minPx, minPy, iw, ih);
            this.ctx.restore();
            this.ctx.beginPath();
            this.ctx.moveTo(pts[0].x, pts[0].y);
            for (let i = 1; i < n; i++) {
            ...
            this.ctx.closePath();
            this.ctx.strokeStyle = tint.stroke;
            this.ctx.lineWidth = 2.5 / this.scale;
            this.ctx.stroke();
        } else {
            this.ctx.fillStyle = tint.fill;
            this.ctx.fill();
            this.ctx.strokeStyle = tint.stroke;
            this.ctx.lineWidth = 2.5 / this.scale;
            this.ctx.lineJoin = 'round';
        ...
        if (isSelected) {
            const r = 4 / this.scale;
            this.ctx.fillStyle = tint.stroke;
            for (const p of pts) {
                this.ctx.fillRect(p.x - r, p.y - r, r * 2, r * 2);
            }
        }
    ...
    drawObject(shape, isInteractive, isSelected) {
        if (isPolygonFootprintObject(shape)) {
            if (shape.subType === 'staircase') {
                this.drawPolygonStaircase(shape, isInteractive, isSelected);
            } else {
                this.drawPolygonFrame(shape, isInteractive, isSelected);
            }
            return;
        }
        ...
        const isStaircaseRect = shape.subType === 'staircase';
        const tint = this.getObjectPlanTint(shape.subType, isInteractive, isSelected);
        const img = shape.imageDataUrl ? this.ensurePlanImage(shape.imageDataUrl) : null;
        ...
        if (img) {
            this.ctx.save();
            this.ctx.beginPath();
            this.ctx.rect(-hw, -hh, localW, localH);
            this.ctx.clip();
            this.ctx.globalAlpha = 1;
            this.ctx.drawImage(img, -hw, -hh, localW, localH);
            this.ctx.restore();
            this.ctx.save();
            this.ctx.strokeStyle = isStaircaseRect
                ? (isSelected ? 'rgba(220, 38, 38, 0.85)' : 'rgba(248, 113, 113, 0.7)')
                : (tint.border || tint.stroke);
            this.ctx.lineWidth = 2 / this.scale;
            ...
            this.ctx.strokeStyle = isInteractive
                ? isStaircaseRect
                    ? (isSelected ? 'rgba(220, 38, 38, 0.92)' : 'rgba(248, 113, 113, 0.85)')
                    : tint.stroke
                : this.exportWallColor;
            this.ctx.lineWidth = 2 / this.scale;
            this.ctx.fillStyle = isInteractive
                ? isStaircaseRect
                    ? (isSelected ? 'rgba(252, 165, 165, 0.38)' : 'rgba(254, 202, 202, 0.28)')
                    : tint.fill
                : (this.isLightBg ? 'rgba(255, 255, 255, 0.8)' : 'rgba(0, 0, 0, 0.4)');

            if (elementDef && elementDef.draw) {
                const colors = {
                    textColor: isInteractive
                        ? isStaircaseRect
                            ? (isSelected ? '#b91c1c' : this.compColor)
                            : (isSelected ? tint.stroke : this.compColor)
                        : this.exportWallColor,
                };
        ...
        if (isSelected) {
            ...
            const r = 4 / this.scale;
            let handleFill = '#6366f1';
            if (shape.subType === 'staircase') handleFill = 'rgba(248, 113, 113, 0.95)';
            else if (['door', 'window', 'hole', 'frame'].includes(shape.subType)) handleFill = tint.stroke;
    ...
    drawShape(shape, isInteractive) {
        ...
        switch (shape.type) {
            ...
            case 'room':
                ...
                break;
            case 'boundary':
                this.drawBoundary(shape, isInteractive, isSelected);
                break;
            case 'object':
    ...
    zoomToFit() {
        ...
        scale = Math.max(this.minViewScale, Math.min(scale, this.maxViewScale));
